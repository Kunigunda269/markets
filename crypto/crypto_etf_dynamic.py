...
def load_config():
    ...
    default_config = {
        ...
        "batch_size": 50,
        "input_file": os.path.join(os.getenv("DATA_DIR", "data"), "category_downloader_123.xlsx"),
        "output_folder": os.getenv("OUTPUT_DIR", "output"),
        "save_path": os.getenv("OUTPUT_DIR", "output"),
...
class DataProcessor:
    ...
    def __init__(self):
        # Базовая директория
        self.base_dir = os.getenv("OUTPUT_DIR", "output")
...
class Visualizer:
    def __init__(self):
        ...
        # Список категорий для исключения из визуализации
        self.excluded_categories = [
            # Существующие категории
            "2017/18 alt season", "ai agent launchpad", "amm", "alameda", "algo stable",
            "alleged sec security", "animoca", "arrington", "asset managment", "brc-20",
            "binance alpha", "binance hodler", "binance launchpad", "binance launchpool",
            "blockchain capital", "bluezilla", "bounce laouch pad", "cmc vote winner",
            "cmc crypto", "cmc labs", "cmc holding", "celsium", "circle venture",
            "coinbase venture", "coinfund portfolio", "destributed computing",
            "doggone doggered", "erc404", "eur stable", "ftx bankruptcy", "fiat stable",
            
            # Дополнительные категории и альтернативные написания
            "2017\\2018 altseason", "2017/2018 altseason", "2017\\18 alt season",
            "celcius bankruptcy estate", "celsius bankruptcy estate",
            "doggone doggorel", "ftx bankrucy estate",
            "oxbull", "parafu capital", "spartna group", "us strategic crypto reserve",
            
            # Категории CMC
            "cmc", "cmc ventures", "cmc community", "cmc staking"
        ]
        
        # Ключевые слова для исключения категорий
        self.excluded_keywords = [
            "portfolio", 
            "launchpad", "lauchpad", 
            "launchpool", "lauchpool", 
            "stable", "стейбл",
            "cmc"  # Добавлено для исключения всех строк, содержащих CMC
        ]

    def filter_excluded_categories(self, categories_df):
        """
        Фильтрация DataFrame категорий, исключая нежелательные категории
        
        Args:
            categories_df: DataFrame с данными категорий
        ...
        # Сохраняем исходное количество категорий для отчета
        original_categories = set(categories_df['Category'].unique())
        print(f"\n=== ФИЛЬТРАЦИЯ КАТЕГОРИЙ ===")
        print(f"Всего категорий до фильтрации: {len(original_categories)}")
        
        # Фильтруем категории из исключаемого списка
        excluded_by_list = set(original_categories).intersection(set(self.excluded_categories))
        filtered_df = categories_df[~categories_df['Category'].isin(self.excluded_categories)]
        
        # Отчет по исключенным категориям из списка
        if excluded_by_list:
            print(f"\nИсключено {len(excluded_by_list)} категорий по списку исключений:")
            for cat in sorted(excluded_by_list):
                print(f"  - {cat}")
        
        # Сохраняем категории после первого фильтра для отчета
        after_list_filter = set(filtered_df['Category'].unique())
        
        # Фильтруем категории по ключевым словам и ведем учет
        excluded_by_keyword = {}
        for keyword in self.excluded_keywords:
            # Сохраняем текущие категории для расчета разницы
            before_filter = set(filtered_df['Category'].unique())
            
            # Применяем фильтр
            filtered_df = filtered_df[~filtered_df['Category'].str.lower().str.contains(keyword.lower(), case=False, na=False)]
            
            # Рассчитываем, какие категории были исключены по данному ключевому слову
            after_filter = set(filtered_df['Category'].unique())
            excluded_with_keyword = before_filter - after_filter
            
            if excluded_with_keyword:
                excluded_by_keyword[keyword] = excluded_with_keyword
        
        # Отчет по исключенным категориям по ключевым словам
        if excluded_by_keyword:
            total_by_keyword = sum(len(cats) for cats in excluded_by_keyword.values())
            print(f"\nИсключено {total_by_keyword} категорий по ключевым словам:")
            
            for keyword, categories in excluded_by_keyword.items():
                print(f"\n  По ключевому слову '{keyword}' исключено {len(categories)} категорий:")
                for cat in sorted(categories):
                    print(f"    - {cat}")
        
        # Итоговая статистика
        remaining_categories = set(filtered_df['Category'].unique())
        removed_categories = original_categories - remaining_categories
        
        print(f"\n=== ИТОГИ ФИЛЬТРАЦИИ ===")
        print(f"Всего категорий до фильтрации: {len(original_categories)}")
        print(f"Исключено категорий: {len(removed_categories)}")
        print(f"Осталось категорий: {len(remaining_categories)}")
        
        print("\nОставшиеся категории для визуализации:")
    ...
    def _add_token_trace(self, fig, token_df, token_symbol):
        ...
        fig.add_trace(go.Scatter(
            x=token_df['date'],
            y=token_df['normalized_price'].tolist(),
            name=f"{token_symbol}",
            mode='lines+markers',
    ...
    def _add_category_traces(self, fig, categories_df, token_df):
        ...
        # Фильтруем категории перед визуализацией
        categories_df = self.filter_excluded_categories(categories_df)
        ...
        for idx, category in enumerate(categories):
            ...
            # Формируем hovertemplate в зависимости от наличия колонки TokenCount
            hovertemplate = (
                f"<b>{category}</b><br>" +
                "Date: %{x}<br>" +
                "Change: %{y:.2f}%<br>"
            ...
            if 'TokenCount' in category_data.columns:
                hovertemplate += "Tokens: {:,d}<br>".format(category_data['TokenCount'].iloc[-1])
            
            hovertemplate += "<extra></extra>"

            fig.add_trace(go.Scatter(
                x=category_data['Date'],
                y=category_data['normalized_price'].tolist(),
                name=f"{category}",
                mode='lines+markers',
                line=dict(
    ...
    def _configure_layout(self, fig, token_symbol, token_change):
        ...
        fig.update_layout(
            # Убираем основной заголовок, чтобы он не перекрывался с нашей аннотацией
            title=None,
            # Удаляем аннотацию с процентами из макета, 
            # так как будем динамически обновлять значения через JavaScript
            yaxis=dict(
                title="Price Change (%)",
                showgrid=True,
                gridcolor='lightgray',
            ...
            hovermode='x unified',
            plot_bgcolor='white',
            margin=dict(t=60, r=200)  # Уменьшенный верхний отступ
        )

        # Мы не добавляем аннотацию на сам график, так как будем использовать
        # только динамический заголовок, созданный в JavaScript коде
        # Это решает проблему с дублированием синей рамки

    def save_plot(self, fig, token_symbol: str):
        """Save plot to HTML file с динамической шапкой"""
        try:
            # Updated save path
            save_path = os.getenv("OUTPUT_DIR", "output")
            timestamp = datetime.now().strftime('%Y%m%d_%H%M')
            ...
            # Удаляем лишний div, если вдруг он есть
            html_content = html_content.replace('<div id="dynamic-header"></div>', '')

            # Новый стиль для шапки
            header_html = '''<div id="dynamic-header"
     style="font-size:2em;font-weight:bold;margin:20px auto 10px auto;padding:10px 0;background:#f8f8f8;border-radius:8px;border:1px solid #e0e0e0;text-align:center;width:100%;max-width:900px;">
    Загрузка...
</div>'''

            # Вставляем div для шапки перед графиком
            insert_pos = html_content.find('<div class="js-plotly-plot"')
            if insert_pos != -1:
                html_content = html_content[:insert_pos] + header_html + html_content[insert_pos:]
            else:
                html_content = html_content.replace('<body>', '<body>\n' + header_html)

            # JS-код для динамической шапки (без изменений)
            custom_js = '''
<script>
document.addEventListener("DOMContentLoaded", function() {
    function findPlotDiv() {
        ...
        if (!plotDiv) {
            ...
            for (let d of divs) {
                if (d.className && d.className.includes('js-plotly-plot')) return d;
            }
        }
        return plotDiv;
    }
    function updateHeader() {
        ...
        if (!plotDiv || !headerDiv) return;
        let data = plotDiv.data || (window.Plotly && window.Plotly.d3 && window.Plotly.d3.select(plotDiv).data()[0]);
        if (!data) return;
        let headerParts = [];
        let anyVisible = false;
        for (let i = 0; i < data.length; i++) {
            let trace = data[i];
            let visible = (typeof trace.visible === 'undefined' || trace.visible === true || trace.visible === 'true');
            if (visible) {
                anyVisible = true;
                let y = Array.isArray(trace.y) ? trace.y[trace.y.length - 1] : null;
                let name = trace.name || `Линия ${i+1}`;
                if (typeof y === 'number' && !isNaN(y)) {
                    let sign = y > 0 ? '+' : '';
                    headerParts.push(`${name}: ${sign}${y.toFixed(2)}%`);
                } else {
                    headerParts.push(`${name}`);
                }
            }
        }
        if (anyVisible && headerParts.length > 0) {
            headerDiv.textContent = headerParts.join(' | ');
        } else {
            headerDiv.textContent = 'Нет выбранных данных';
        }
    }
    setTimeout(updateHeader, 500);
    let plotDiv = findPlotDiv();
    if (plotDiv) {
        plotDiv.on('plotly_legendclick', function() { setTimeout(updateHeader, 100); });
        plotDiv.on('plotly_legenddoubleclick', function() { setTimeout(updateHeader, 100); });
        plotDiv.on('plotly_restyle', function() { setTimeout(updateHeader, 100); });
        plotDiv.on('plotly_afterplot', function() { setTimeout(updateHeader, 100); });
    }
    window.addEventListener('resize', function() { setTimeout(updateHeader, 200); });
});
</script>
'''
...
class S3Uploader:
    ...
    def upload_file(self, file_path: str, object_name: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            url = f"{S3_URL}/{self.bucket_name}/{object_name}"
            logging.info(f"Файл успешно загружен в S3: {url}")
            return True, url
        except Exception as e:
...
def save_token_data(token_df: pd.DataFrame, token_symbol: str) -> str:
    ...
    try:
        ...
        save_dir = os.getenv("OUTPUT_DIR", "output")
...
async def save_and_visualize(token_df, categories_df, token_info, data_processor, visualizer):
    ...
    visualization_result = await create_visualization(token_df, categories_df, token_info, visualizer)
    ...
    if visualization_result:
        if visualization_result.get("local_file"):
            print(f"3. Интерактивный график (локально): {visualization_result['local_file']}")
            
        if visualization_result.get("s3_url"):
            print(f"4. Интерактивный график (онлайн): {visualization_result['s3_url']}")
...
async def create_visualization(token_df, categories_df, token_info, visualizer):
    ...
    # Сохраняем график
    filename = visualizer.save_plot(fig, token_info['symbol'])
    ...
    if filename and os.path.exists(filename):
        ...
        # Создаем экземпляр S3Uploader и загружаем файл
        s3_uploader = S3Uploader()
        success, s3_url = s3_uploader.upload_file(filename)
        
        if success:
            print(f"График успешно загружен в S3: {s3_url}")
        else:
            print(f"Ошибка при загрузке в S3: {s3_url}")
            
        return {
            "local_file": filename,
            "s3_url": s3_url if success else None
        }
    else:
