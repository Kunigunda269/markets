    ...
    async with session.get(url, headers=HEADERS, params=params) as response:
        if response.status == 200:
            ...
            if len(quotes) >= 2:
                ...
                price_change = price_change_percentage(open_price, close_price)
                logging.info(f"Token ID: {crypto_id}, Change: {price_change}%")
                return price_change
        return None


async def fetch_top_100_cryptos(session, time_start, time_end):
    """Получение топ-100 криптовалют"""
...
def plot_heatmap(df, time_start, time_end):
    """Создание оптимизированной адаптивной тепловой карты"""
    ...
    # Логарифмическая шкала для размеров
    df['size_factor'] = np.log10(df['market_cap'])
    df = df.sort_values('market_cap', ascending=False)

    # Определяем уровни отображения текста
    ...
    html_file = os.path.join(os.getenv("DATA_DIR", "data"), "heatmap.html")
    with open(html_file, "w", encoding="utf-8") as f:
        ...
        <head>
            ...
            <style>
                body { margin: 0; }
                #heatmap { width: 100%; height: 100vh; }
            </style>
        </head>
        <body>
            <div id="heatmap"></div>
            <script>
                var fig = """ + fig.to_json() + """;
                Plotly.newPlot('heatmap', fig.data, fig.layout, {responsive: true});
    ...
    fig.show()
    fig.write_html(
        os.path.join(os.getenv("DATA_DIR", "data"), "heatmap.html"),
